...
function evolveReadyWeapons(): boolean {
  ...
  state.weapons.forEach((weapon) => {
    if (weapon.evolved || weapon.level < weapon.maxLevel || !hasPassive(weapon.evolution.requiredPassive)) return;
...
function grantChestReward(chest: Chest): void {
  ...
  if (chest.kind === "cache") {
    ...
    if (!evolveReadyWeapons()) upgradeRandomWeapon();
...
function upgradeRandomWeapon(): boolean {
  ...
  if (candidates.length === 0) return evolveReadyWeapons();
...
function updateHud(forceRail = false): void {
  ...
  if (forceRail) {
    ui.rail.innerHTML = state.weapons
      .map(
        (weapon) =>
          `<span class="weapon-pill"><i class="weapon-dot" style="color:${weapon.color}"></i>${weapon.name} ${weapon.level}</span>`,
      )
      .join("");
  }
}
